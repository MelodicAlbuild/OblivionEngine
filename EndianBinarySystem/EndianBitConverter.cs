...
public static class EndianBitConverter
{
    ...
    public static unsafe byte[] Int16ToBytes(short value, Endianness targetEndianness)
    {
        ...
        if (SystemEndianness != targetEndianness) FlipPrimitives(bytes, 0, 1, 2);
        return bytes;
    }

    public static unsafe byte[] Int16sToBytes(short[] value, int startIndex, int count, Endianness targetEndianness)
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count)) return Array.Empty<byte>();
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            array = new byte[2 * count];
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((short*)b)[i] = value[startIndex + i];
            }

            if (SystemEndianness != targetEndianness) FlipPrimitives(array, 0, count, 2);
        }
    ...
    public static unsafe byte[] UInt16sToBytes(ushort[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((ushort*)b)[i] = value[startIndex + i];
            }
    ...
    public static unsafe byte[] Int32ToBytes(int value, Endianness targetEndianness)
    {
        ...
        if (SystemEndianness != targetEndianness) FlipPrimitives(bytes, 0, 1, 4);
        return bytes;
    }
    ...
    public static unsafe byte[] Int32sToBytes(int[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((int*)b)[i] = value[startIndex + i];
            }

            if (SystemEndianness != targetEndianness) FlipPrimitives(array, 0, count, 4);
        }
    ...
    public static unsafe byte[] UInt32sToBytes(uint[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((uint*)b)[i] = value[startIndex + i];
            }
    ...
    public static unsafe byte[] Int64ToBytes(long value, Endianness targetEndianness)
    {
        ...
        if (SystemEndianness != targetEndianness) FlipPrimitives(bytes, 0, 1, 8);
        return bytes;
    }
    ...
    public static unsafe byte[] Int64sToBytes(long[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((long*)b)[i] = value[startIndex + i];
            }

            if (SystemEndianness != targetEndianness) FlipPrimitives(array, 0, count, 8);
        }
    ...
    public static unsafe byte[] UInt64sToBytes(ulong[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((ulong*)b)[i] = value[startIndex + i];
            }
    ...
    public static unsafe byte[] SinglesToBytes(float[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((float*)b)[i] = value[startIndex + i];
            }
    ...
    public static unsafe byte[] DoublesToBytes(double[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((double*)b)[i] = value[startIndex + i];
            }
    ...
    public static unsafe byte[] DecimalToBytes(decimal value, Endianness targetEndianness)
    {
        ...
        if (SystemEndianness != targetEndianness) FlipPrimitives(bytes, 0, 1, 16);
        return bytes;
    }
    ...
    public static unsafe byte[] DecimalsToBytes(decimal[] value, int startIndex, int count, Endianness targetEndianness)
    {
        ...
        if (!Utils.ValidateReadArraySize(count, out byte[] array))
        {
            ...
            fixed (byte* b = array)
            {
                for (var i = 0; i < count; i++) ((decimal*)b)[i] = value[startIndex + i];
            }

            if (SystemEndianness != targetEndianness) FlipPrimitives(array, 0, count, 16);
        }
    ...
    {
        if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, 1, 2);
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 2)) return Array.Empty<short>();
        if (!Utils.ValidateReadArraySize(count, out short[] array))
        {
            if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, count, 2);
            array = new short[count];
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((short*)b)[i];
            }
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 2)) return Array.Empty<ushort>();
        if (!Utils.ValidateReadArraySize(count, out ushort[] array))
        {
            ...
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((ushort*)b)[i];
            }
        }
    ...
    {
        if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, 1, 4);
        fixed (byte* b = &value[startIndex])
        {
            return *(int*)b;
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 4)) return Array.Empty<int>();
        if (!Utils.ValidateReadArraySize(count, out int[] array))
        {
            if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, count, 4);
            array = new int[count];
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((int*)b)[i];
            }
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 4)) return Array.Empty<uint>();
        if (!Utils.ValidateReadArraySize(count, out uint[] array))
        {
            ...
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((uint*)b)[i];
            }
        }
    ...
    {
        if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, 1, 8);
        fixed (byte* b = &value[startIndex])
        {
            return *(long*)b;
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 8)) return Array.Empty<long>();
        if (!Utils.ValidateReadArraySize(count, out long[] array))
        {
            if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, count, 8);
            array = new long[count];
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((long*)b)[i];
            }
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 8)) return Array.Empty<ulong>();
        if (!Utils.ValidateReadArraySize(count, out ulong[] array))
        {
            ...
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((ulong*)b)[i];
            }
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 4)) return Array.Empty<float>();
        if (!Utils.ValidateReadArraySize(count, out float[] array))
        {
            ...
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((float*)b)[i];
            }
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 8)) return Array.Empty<double>();
        if (!Utils.ValidateReadArraySize(count, out double[] array))
        {
            ...
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((double*)b)[i];
            }
        }
    ...
    {
        if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, 1, 16);
        fixed (byte* b = &value[startIndex])
        {
            return *(decimal*)b;
        }
    ...
    {
        if (Utils.ValidateArrayIndexAndCount(value, startIndex, count * 16)) return Array.Empty<decimal>();
        if (!Utils.ValidateReadArraySize(count, out decimal[] array))
        {
            if (SystemEndianness != sourceEndianness) FlipPrimitives(value, startIndex, count, 16);
            array = new decimal[count];
            fixed (byte* b = &value[startIndex])
            {
                for (var i = 0; i < count; i++) array[i] = ((decimal*)b)[i];
            }
        }
    ...
    {
        var byteCount = primitiveCount * primitiveSize;
        for (var i = startIndex; i < byteCount + startIndex; i += primitiveSize)
        {
            var a = i;
            var b = i + primitiveSize - 1;
            while (a < b)
            {
                var by = buffer[a];
                buffer[a] = buffer[b];
